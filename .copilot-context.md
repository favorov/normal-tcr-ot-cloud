...
for b in range(bootstrap_n):
    boot_samples = sample(cloud_samples, replace=True, size=n)
    grid_b, bary_b = barycenter(boot_samples)
    ...
    null.extend(dist(s, bary_b) for s in subset)

# 4) Save as text (one value per line)
