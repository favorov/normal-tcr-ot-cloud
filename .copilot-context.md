...
load_distribution(filepath, freq_column="pgen", weights_column="duplicate_frequency_percent")
...
4. Normalize: ensure weights sum to 1
5. Discretize onto common grid (for consistency)
6. Compute Wasserstein distance using EMD solver
...
# One-to-all
compute_distance_one_to_all(ref_file, ...)

# All-pairs (upper triangle only)
compute_distance_all_pairs(...)
```
...
cd input/test-cloud-Tumeh2014 && \
python3 ../../olga-p2p-ot.py --all --statistics-only
```
