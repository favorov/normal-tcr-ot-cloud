...
Computes p-values for samples based on their distance to a barycenter using empirical null distribution or normal approximation.
...
- `--vdj-filter` — require non-empty `v_call`, `d_call`, `j_call` for columns that exist
- `--vj-filter` — require non-empty `v_call`, `j_call` for columns that exist
- `--null-distribution <file>` — path to bootstrap null distribution (default: looks for p2b-ot-null.txt in barycenter folder)
- `--normal-approximation` — also compute normal-approximation p-values; if no null distribution is available, normal approximation becomes the only method
- `--no-null-distribution` — disable null distribution, use only normal approximation

### How it works

**Default behavior (with empirical null distribution):**

1. Loads null distribution from `p2b-ot-null.txt` (or specified file)
2. Computes distances for all test samples to barycenter
3. For each sample, computes p-value as: (number of null values ≥ observed distance) / (total null values)
   - For distances beyond max(null), uses lower bound: 1 / (number of null values)
4. Applies Bonferroni correction to account for multiple testing
5. Displays results sorted by corrected p-value

**With normal approximation option:**

1. If null distribution is available, the script computes both empirical-null and normal-approximation p-values
2. If null distribution is not available, the script uses normal approximation only
3. Normal p-values are computed as two-tailed probabilities under the fitted normal model
4. Bonferroni correction is applied separately for each p-value family
5. Results table includes both sets of p-values when both methods are active
...
- Default single-method output: `P-value`, `Bonf. p`
- Dual-method output (`--normal-approximation` with available null distribution): `P-val(null)`, `Bonf.(null)`, `P-val(norm)`, `Bonf.(norm)`

Summary statistics:
- P-value computation method or methods used
...
```bash
# Default: use bootstrap null distribution if available
python3 olga-samples-p2b-pval.py input/test-cloud-Tumeh2014 input/new-samples

# Use specific null distribution file
python3 olga-samples-p2b-pval.py input/test-cloud-Tumeh2014 input/new-samples \
    --null-distribution custom_null.txt

# Compute both empirical-null and normal-approximation p-values
python3 olga-samples-p2b-pval.py input/test-cloud-Tumeh2014 input/new-samples \
    --normal-approximation

# Disable null distribution entirely
...
**Empirical null distribution (recommended):**
- Generated by bootstrap (see `olga-barycenter-ot-bootstrap.py`)
- Non-parametric: makes no distributional assumptions
- p-value = P(null ≥ observed) = upper-tail probability
- More powerful for capturing complex null hypothesis shapes

**Normal approximation (fallback):**
- Fits normal distribution to barycenter sample distances
- Parametric: assumes normality
- Automatic fallback if null distribution file not found
- When requested together with an available null distribution, its p-values are shown alongside empirical-null p-values
- Useful for exploratory analysis or when null distribution unavailable

### Notes on multiple testing correction
