...
def compute_distance_single_pair(input_folder, file1, file2, freq_column, weights_column, n_grid):
...
def compute_distance_one_to_all(input_folder, ref_file, freq_column, weights_column, n_grid):
    ...
    for other_file in all_files:
        ...
        grid = create_common_grid([ref_values, other_values], n_grid=n_grid, log_space=True)
...
def compute_distance_all_pairs(input_folder, freq_column, weights_column, n_grid):
        ...
        if all_mode:
            if file1 is not None:
                ...
                if not pipeline_mode:
                    ...
                    print()
                results = compute_distance_one_to_all(input_folder, file1, freq_column, weights_column, n_grid)
                if not pipeline_mode:
                    if statistics_only:
                ...
                if not pipeline_mode:
                    ...
                    print()
                results = compute_distance_all_pairs(input_folder, freq_column, weights_column, n_grid)
                if not pipeline_mode:
                    if statistics_only:
